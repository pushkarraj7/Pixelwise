...
//   const handleNextSmall = () => {
//     setVisibleStart((prev) => Math.min(cards.length - 3, prev + 1));
//   };
...
//       <div className="mt-4 sm:mt-2 lg:mt-10 flex flex-col items-center justify-center w-full max-w-4xl">
//         <div className="flex items-center justify-center w-full overflow-hidden relative">
//           <button
//             onClick={handlePrevSmall}
//             className={`text-2xl pr-4 cursor-pointer text-white transition-opacity duration-300 ${
...
//           <button
//             onClick={handleNextSmall}
//             // disabled={visibleStart >= cards.length - 3}
//             className={`text-2xl pl-4 cursor-pointer text-white transition-opacity duration-300 ${
//               visibleStart >= cards.length - 4
//                 ? "opacity-50 cursor-not-allowed"
//                 : "opacity-100"
...
//         <button
//           onClick={() => setIsModalOpen(true)}
//           className="fixed bottom-16 lg:bottom-4 right-4 bg-[#34CCCC] text-white px-4 py-2 sm:px-6 sm:py-3 rounded-full shadow-lg cursor-pointer flex items-center gap-1 sm:gap-2 text-sm sm:text-base"
//         >
//           <svg
...
//             <path d="M15 12a1 1 0 0 0 .962-.726l2-7A1 1 0 0 0 17 3H3.77L3.175.745A1 1 0 0 0 2.208 0H1a1 1 0 0 0 0 2h.438l.6 2.255v.019l2 7 .746 2.986A3 3 0 1 0 9 17a2.966 2.966 0 0 0-.184-1h2.368c-.118.32-.18.659-.184 1a3 3 0 1 0 3-3H6.78l-.5-2H15Z" />
//           </svg>
//           <span className="text-xs sm:text-sm">({selectedCards.length})</span>
//         </button>
...
export default function FeatureShowcase() {
  ...
  useEffect(() => {
    sessionStorage.setItem("cartData", JSON.stringify(selectedCards));
  }, [selectedCards]);

  const handleCardSelection = (card) => {
    setSelectedCards((prev) => {
      const updatedCards = [...prev, card];
      sessionStorage.setItem("cartData", JSON.stringify(updatedCards));
      return updatedCards;
    });
  };
        ...
        {isModalOpen && (
          <>
            ...
            <div className="fixed top-0 right-0 h-full max-h-screen w-full sm:w-2/3 md:w-1/2 lg:w-1/3 bg-white shadow-lg p-6 overflow-y-auto text-black z-50 transition-transform duration-300">
              ...
              <h2 className="text-lg font-bold mb-4">Selected Features</h2>
              {/* Show "Cart is empty" if no items */}
